...
class puzzle:
    ...
    def solve(self, block, left, right, bottom, topr):
        ...
        if topr - bottom <= 1:
            ...
            if quad == 1:
                ...
                q3 = (cB, cL)
                q4 = (cB, cL+1)
                self.solve(q1,cL+1,right,cB+1,topr)
                self.solve(q2,left,cL,cB+1,topr)
                self.solve(q3,left,cL,bottom,cB)
                self.solve(q4,cL+1,right,bottom,cB)
    ...
    def get_quadrant(self, block, left, right, bottom, topr):
        # Your code goes here
        if block[1] >= ((right + left) // 2+1):
            if block[0] >= ((topr + bottom) // 2+1):
                return 1
            else: 
                return 4 
        else: 
